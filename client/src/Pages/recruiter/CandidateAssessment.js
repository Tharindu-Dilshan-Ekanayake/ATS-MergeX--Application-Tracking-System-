...
export default function CandidateAssessment() {
  ...
  const [unchekedEvalautionsById, setUncheckedEvaluationsById] = useState([]);
  const [uncheckedCandidatesById, setUncheckedCandidatesById] = useState([]);
...
const uncheckedgetImgByJobId = async (user_id) => {
  if (!user_id) {
    console.error("User ID is required");
    return;
  }
  try {
    ...
    setUncheckedCandidatesById((prevState) =>
      prevState.map((candidate) =>
        candidate.userid === user_id
          ? { ...candidate, image: response.data.image }
          : candidate
  ...
  const processUncheckedCandidatesByJobId = async () => {
    unchekedEvalautionsById.forEach((evoluation) => {
      const { _id, candidateid, candidatename, candidateemail, position } =
        evoluation;
      setUncheckedCandidatesById((prevState) => [
        ...prevState,
      ...
      uncheckedgetImgByJobId(candidateid);
    });
  };
  ...
  useEffect(() => {
    ...
    fetchEvaluation();
  }, [showDetails, selected,checkedEvaluationsbyId,unchekedEvalautionsById,checkedCandidates,uncheckedCandidatesById,checkedCandiatesByJobId]);
...
const getUncheckedEvaluatuonsbyId = async (job_id) => {
  try {
    const response = await axios.get(
      `http://localhost:8000/evaluation/getRecruiterUnCheckedEvaluationsbyID/${job_id}`
    );
    setUncheckedEvaluationsById(response.data);
...
const getcheckedEvaluatuonsbyId = async (job_id) => {
  try {
    const response = await axios.get(
      `http://localhost:8000/evaluation/getRecruitercheckedEvaluationsbyID/${job_id}`
    );
...
useEffect(() => {
  ...
  useEffect(() => {
    processUncheckedCandidatesByJobId();
  }, [unchekedEvalautionsById]);
...
if(showEvaluated){
  ...
  return (
    ...
    className={`content ml-[340px]  text-white  p-[0px]  h-[90vh] bg-[#212121]  rounded-[30px] 320px:text-[0.5rem]  450px:text-[0.8rem] sm:text-[0.9rem]   900px:text-[1.1rem]  1010px:text-[1.2rem]  overflow-hidden  ${
...
{showApprovedjobPosting ? (
    <div
      className={`content  text-white flex flex-row p-[0px]   h-[90vh]  m-[30px] mt-[5px]  h-fit rounded-[30px] 320px:text-[0.5rem]  450px:text-[0.8rem] sm:text-[0.9rem]   900px:text-[1.1rem]  1010px:text-[1.2rem]  ${
        showDetails === false ? " justify-center " : null
      }`}
      // bg-[#212121]
    >
            ...
            <div>
              {approvedjobPosting.map((job, index) => (
                <button
                  ...
                  onClick={() => {
                    setShowApprovedjobPosting(false);
                    getUncheckedEvaluatuonsbyId(job._id);
      ...
      {showDetails ? (
        <div className="description flex flex-col w-full pt-[20px] box-border h-[85vh] overflow-auto overflow-x-hidden ">
          ...
          <div className="">
            ...
            <button
              type="submit"
              onClick={(e)=>{updateEvaluation(e);setshowDetails(false);setShowApprovedjobPosting(true);}}
    ...
    {!showDetails ? ( 
         <div> 
          ...
          className="absolute right-[60px] top-[120px] w-[50px] h-[50px] text-[#EA7122]"
        />)} 
                {!clicked ? ( <p className="absolute top-[50%] left-[50%] text-[#a3a3a3] text-center text-[28px] ">
